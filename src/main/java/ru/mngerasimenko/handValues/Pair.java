...
public class Pair extends HandValue {
	...
	public Pair(List<Card> pokerHand) {
	...
	public int compareTo(HandValue handValue) {
		...
		compare = this.kicker.get(1).compareTo(handValue.getKicker().get(1));
		if (compare != 0) {
			return compare;
		}
		return this.kicker.get(2).compareTo(handValue.getKicker().get(2));
	}
